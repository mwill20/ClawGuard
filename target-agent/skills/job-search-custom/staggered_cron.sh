...
EMAIL_TO=$(get_env "CLAWGUARD_EMAIL_TO")
RUNTIME_EVENTS_ENABLED=$(get_env "CLAWGUARD_RUNTIME_EVENTS_ENABLED")

AUTO_PREPARE_THRESHOLD="${AUTO_PREPARE_THRESHOLD:-0.75}"
ENRICHMENT_DAILY_CAP="${ENRICHMENT_DAILY_CAP:-0}"
DIGEST_MAX_RESULTS_PER_SITE="${DIGEST_MAX_RESULTS_PER_SITE:-20}"
...
if [ "$SITE" = "compile" ]; then
    ...
    docker exec \
      ...
      -e CLAWGUARD_EMAIL_TO="$EMAIL_TO" \
      -e CLAWGUARD_RUNTIME_EVENTS_ENABLED="$RUNTIME_EVENTS_ENABLED" \
