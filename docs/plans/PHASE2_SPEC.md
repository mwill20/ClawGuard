...
The March 2026 project specs are useful history, but they describe a project that no longer exists. Phase 1 and Phase 2 are closed. The repo now has a live OpenClaw telemetry loop, detector-backed ASI06, ASI01, and ASI02 modules, session-correlated SQLite findings, post-compile telemetry, curated export helpers, runtime-event contract validation, runtime-event writer tests, runtime-event redaction/export tests, host annotation tests, normal-ops runtime-event fixture validation, source-health digest reporting, lessons, CI, and 81 local tests.
